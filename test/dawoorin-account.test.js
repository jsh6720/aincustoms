...
test("Dawoorin migration provisions a disabled destination account without a shared password", () => {
  ...
  assert.match(
    migration,
    /extensions\.crypt\(pg_catalog\.gen_random_uuid\(\)::text,\s*extensions\.gen_salt\('bf'\)\)/i,
  );
  ...
  assert.match(migration, /'destination',\s*false\s*\)/i);
  assert.doesNotMatch(
    migration,
    /else[\s\S]{0,700}password_hash\s*=/i,
  );
