...
const migrationPath = path.join(
  root, "supabase/migrations/20260903090000_add_missing_quarantine_notifications.sql"
);
const importRequestHandlerPath = path.join(root, "api/cargo-import-request.js");

function loadMailBuilder() {
  return require("../lib/cargo-missing-quarantine-notification");
}
...
const snapshot = {
  ...
  missing_quarantine_fields: ["동물검역 신고", "식품검역 신고"],
};

test("migration allows the daily missing quarantine event and removes manual O", () => {
  assert.equal(fs.existsSync(migrationPath), true);
  const migration = fs.readFileSync(migrationPath, "utf8");
  assert.match(migration, /quarantine_declaration_missing/);
  assert.match(migration, /animal_quarantine_override\s*=\s*null/i);
  assert.match(migration, /food_quarantine_override\s*=\s*null/i);
  assert.match(migration, /20260903090000/);
});

test("quarantine reminder mail lists the warehouse date and missing declarations", () => {
  const { buildMissingQuarantineMail } = loadMailBuilder();
  const mail = buildMissingQuarantineMail(snapshot);
  assert.equal(mail.subject, "[검역신고 확인] 현대코퍼레이션H_BL001 / 캐틀팜");
  assert.match(mail.text, /입고\(예정\)일: 2026-09-04/);
  assert.match(mail.text, /동물검역 신고/);
  assert.match(mail.text, /식품검역 신고/);
});

test("signed quarantine event sends once to the four internal recipients", { concurrency: false }, async () => {
  ...
  const handler = loadImportRequestHandler({
    ...
    supabaseFetch: async (url, options = {}) => {
      if (url.includes("/rpc/claim_cargo_automatic_mail")) {
        ...
        assert.deepEqual(payload.p_allowed_event_types, ["quarantine_declaration_missing"]);
        return [{
          id: "event-quarantine", event_type: "quarantine_declaration_missing",
          status: "sending", claim_token: "claim-quarantine", claimed: true,
          card_snapshot: snapshot,
        }];
      }
      if (url.includes("/rpc/settle_cargo_mail")) {
        settlements.push(JSON.parse(options.body));
        return [{ id: "event-quarantine", settled: true, status: settlements.at(-1).p_status }];
      }
  ...
  await withEnvironment({
    ...
    headers: {
      ...
      "x-cargo-sync-signature": createSyncSignature(secret, timestamp, "event-quarantine"),
    },
    body: { action: "auto_missing_quarantine_notice", event_id: "event-quarantine" },
  }, response));
