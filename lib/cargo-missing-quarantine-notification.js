...
function buildMissingQuarantineMail(snapshot = {}) {
  ...
  const missingFields = Array.isArray(snapshot.missing_quarantine_fields)
    ? snapshot.missing_quarantine_fields
  ...
  const lines = [
    "안녕하세요. 아인합동관세사입니다.",
    "",
    "입고(예정)일이 2일 이내로 남았으나 아래 검역신고 작성 여부가 확인되지 않았습니다.",
    "",
    `화주명: ${consignee}`,
    `B/L: ${blNumber}`,
    `납품처: ${destination}`,
    ...
    "",
    "확인 필요 항목:",
    ...(missingFields.length ? missingFields : ["동물검역 신고", "식품검역 신고"])
      .map((field) => `- ${field}`),
    "",
    "검역신고 작성 여부를 확인하여 대시보드에 반영해 주시기 바랍니다.",
    "",
    "감사합니다.",
    "아인합동관세사무소",
  ];
  ...
  return {
    subject: `[검역신고 확인] ${consignee}_${blNumber} / ${destination}`,
    text,
    html: mailTextToHtml(text),
  };
}

module.exports = { buildMissingQuarantineMail };
