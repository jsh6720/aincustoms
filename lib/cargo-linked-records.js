...
function mergeOriginalDocRows(rows) {
  ...
  const receiptRows = items.filter((item) => (
    item.obl_received === true || item.hc_received === true
  ));
  const receiptOrigin = receiptRows.find((item) => (
    text(item.updated_by) !== "linked-account-sync"
  )) || receiptRows[0] || newest;
  const transferRow = items.find((item) => (
    item.transfer_received_override === true ||
    item.transfer_received_override === false
  ));
  const transferOrigin = transferRow
    ? items.find((item) => (
      item.transfer_received_override === transferRow.transfer_received_override
      && text(item.updated_by) !== "linked-account-sync"
    )) || transferRow
    : null;
  return {
    ...
    transfer_received_override: transferRow
      ...
      : null,
    transfer_received_updated_at: transferOrigin?.updated_at || "",
    transfer_received_updated_by: transferOrigin?.updated_by || "",
    ...
    receipt_updated_at: receiptOrigin.updated_at || newest.updated_at || "",
  };
}
...
function mergedDeliveryField(rows, statusField, dateField, timestampField) {
  const enabledRows = newestFirst(
    rows.filter((item) => item?.[statusField] === true),
    "updated_at"
  );
  const datedRow = enabledRows.find((item) => text(item?.[dateField]));
  const timestampedRow = enabledRows.find((item) => text(item?.[timestampField]));
  return {
    status: enabledRows.length > 0,
    date: datedRow ? text(datedRow[dateField]) : "",
    timestamp: timestampedRow ? text(timestampedRow[timestampField]) : "",
  };
}

function mergeLinkedDeliveryStatus(card, cardRefs, inputs) {
  const rows = linkedRows(card, cardRefs, inputs);
  const newest = newestFirst(rows, "updated_at")[0];
  const samhyeon = mergedDeliveryField(
    rows,
    "docs_delivered_samhyeon",
    "docs_delivered_samhyeon_date",
    "docs_delivered_samhyeon_at"
  );
  const warehouse = mergedDeliveryField(
    rows,
  ...
  return {
    docs_delivered_samhyeon: samhyeon.status,
    docs_delivered_samhyeon_date: samhyeon.date,
    docs_delivered_samhyeon_at: samhyeon.timestamp,
    docs_delivered_warehouse: warehouse.status,
    docs_delivered_warehouse_date: warehouse.date,
    docs_delivered_warehouse_at: warehouse.timestamp,
    docs_delivery_updated_at: newest?.updated_at || null,
  };
}
