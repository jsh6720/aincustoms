...
function mergeLinkedDeliveryStatus(card, cardRefs, inputs) {
  const newest = newestFirst(linkedRows(card, cardRefs, inputs), "updated_at")[0];
  return {
    docs_delivered_samhyeon: newest?.docs_delivered_samhyeon === true,
    docs_delivered_warehouse: newest?.docs_delivered_warehouse === true,
