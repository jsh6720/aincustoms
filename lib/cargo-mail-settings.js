...
function isMailDirectoryAccount(account) {
  if (account?.is_active === false) return false;
  const role = String(account?.role || "shipper").trim().toLowerCase();
...
function resolveAccountDirectoryNoticeRecipients({ accounts, card, ainRecipients } = {}) {
  ...
  const directoryAccounts = (accounts || []).filter(isMailDirectoryAccount);
  const shipperAccounts = directoryAccounts.filter((account) =>
    accountMatchesParty(account, card?.consignee)
  );
  const destinationAccounts = directoryAccounts.filter((account) =>
...
async function fetchMailDirectoryAccounts(supabaseFetch) {
  return await supabaseFetch(
    "/rest/v1/shipper_accounts?select=id,login_id,display_name,consignee_filter,release_request_to,role,account_category,is_active&is_active=eq.true"
  );
}
