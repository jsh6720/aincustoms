...
function buildChangeHtmlLine(label, previousValue, nextValue) {
  ...
  return `${escapeHtml(label)}: ${escapeHtml(before)} <strong>→ ${escapeHtml(after)}</strong>`;
}
...
function buildWarehouseChangeMail(card, session, previous, next) {
  ...
  const lines = [
    ...
    buildChangeLine("반입예정구역", before.storage_yard, after.storage_yard),
    buildChangeLine(
      "반입예정일",
      before.warehouse_expected_date,
      after.warehouse_expected_date
    ),
  ];
  return {
    ...
    html: [
      ...
      escapeHtml(lines[3]),
      escapeHtml(lines[4]),
      buildChangeHtmlLine("반입예정구역", before.storage_yard, after.storage_yard),
      buildChangeHtmlLine(
        "반입예정일",
        before.warehouse_expected_date,
        after.warehouse_expected_date
