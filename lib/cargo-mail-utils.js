...
function highlightedTransportLine(line, options = {}) {
  ...
  const customsArrival = raw.match(/^(입항일:\s*)(.+\(관세청 확인\))$/);
  if (customsArrival && options.highlightCustomsArrival !== false) {
    return `${escapeHtml(customsArrival[1])}<strong style="${CHANGE_HIGHLIGHT_STYLE}">${escapeHtml(customsArrival[2])}</strong>`;
  }
...
function mailTextToHtml(text, options = {}) {
  const content = String(text || "")
    .split("\n")
    .map((line) => options.highlightChanges ? highlightedTransportLine(line, options) : escapeHtml(line))
    .join("<br>");
...
function buildDetailedTransportMail(card, previous, next, subject, options = {}) {
  ...
  const customsArrivalConfirmed = next?.arrival_confirmed_by_customs === true;
  const customsArrivalIsFocus = customsArrivalConfirmed && options.focus === "arrival";
  const intro = customsArrivalIsFocus
    ? "관세청 전산에서 실제 입항이 확인되어 아래와 같이 안내드립니다."
    : "반입예정정보가 변경되어 아래와 같이 안내드립니다.";
  const arrivalTextLine = customsArrivalConfirmed
    ? `입항일: ${nextEta || "미입력"} (관세청 확인)`
    : buildChangeLine("입항예정일", previousEta, nextEta);
  const arrivalHtmlLine = customsArrivalIsFocus
    ? `${escapeHtml("입항일")}: <strong style="${CHANGE_HIGHLIGHT_STYLE}">${escapeHtml(`${nextEta || "미입력"} (관세청 확인)`)}</strong>`
    : customsArrivalConfirmed
      ? escapeHtml(arrivalTextLine)
...
function buildArrivalScheduleChangeMail(card, previous, next) {
  ...
  const destination = destinationName(card?.destination);
  const subjectPrefix = next?.arrival_confirmed_by_customs === true
    ? "[입항 확인]"
    : "[입항 스케줄 변경]";
  return buildDetailedTransportMail(
    ...
    previous,
    next,
    `${subjectPrefix} ${subjectConsignee}_${blNumber} / ${destination}`,
    { focus: "arrival" }
  );
}
