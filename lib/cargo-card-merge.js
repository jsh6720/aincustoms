...
function mergeGroup(cards) {
  ...
  const newestOblCarrierCard = cards.reduce((latest, card) => {
    if (!card.obl_carrier_submitted_at) return latest;
    if (!latest) return card;
    return timestamp(card.obl_carrier_submitted_at) >= timestamp(latest.obl_carrier_submitted_at)
      ? card
      : latest;
  }, null);
  const legacyOblCarrierCard = newestOblCarrierCard
    ? null
    : cards.find((card) => (
      card.obl_carrier_submitted === true && card.obl_carrier_submitted_date
    ));
  copyPresentFields(
    merged,
    newestOblCarrierCard || legacyOblCarrierCard,
    OBL_CARRIER_FIELDS
  );
